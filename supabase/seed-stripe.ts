  ...
  if (events) {
    events.forEach(async event => {
      ...
      if (!existingProduct) {
        ...
        if (fetchError) {
          console.error(`Error fetching event ${eventIdShort} to check price_stripe_id:`, fetchError);
          return;
        }
        ...
        if (!eventWithPrice.price_stripe_id) {
          ...
          try {
            ...
            console.error(`[Error] fetching related product for event ${eventIdShort} with prod_${event.id}:`, productError);
            return;
          }

          if (!relatedProduct) {
            console.error(`[Error] No related product found for event ${event.id} with prod_${event.id}`);
            return;
          }
          ...
          if (priceId) {
            ...
            if (updateError) {
              console.error(`[Failed] to update event ${eventIdShort} with Stripe price ID:`, updateError);
